...
addon = xbmcaddon.Addon('plugin.video.pmsf.iptv')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.pmsf-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(pmsf,We failed with error code - "+str(e.code)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(pmsf,New source added.,5000,"+icon+")")
        if not url is None:
